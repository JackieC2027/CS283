...
int get_student(int fd, int id, student_t *s){
    ...
    off_t studentDatabasePosition = id * STUDENT_RECORD_SIZE;
    if(lseek(fd, studentDatabasePosition, SEEK_SET) == -1){
        return ERR_DB_FILE;
    }
    ssize_t studentBytesFromDatabaseFile = read(fd, s, sizeof(student_t));
    if(studentBytesFromDatabaseFile == -1){
        return ERR_DB_FILE;
    }
    if(studentBytesFromDatabaseFile != sizeof(student_t)){
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    student_t newStudentEntry;
    newStudentEntry.id = id;
    strncpy(newStudentEntry.fname, fname, sizeof(newStudentEntry.fname) - 1);
    newStudentEntry.fname[sizeof(newStudentEntry.fname) - 1] = '\0';
    strncpy(newStudentEntry.lname, lname, sizeof(newStudentEntry.lname) - 1);
    newStudentEntry.lname[sizeof(newStudentEntry.lname) - 1] = '\0';
    newStudentEntry.gpa = gpa;
    student_t currentStudentEntry;
    ssize_t studentBytesFromDatabaseFile = read(fd, &currentStudentEntry, sizeof(student_t));
    if(studentBytesFromDatabaseFile == -1){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    if (studentBytesFromDatabaseFile > 0 && memcmp(&currentStudentEntry, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
    ...
    ssize_t newStudentBytes = write(fd, &newStudentEntry, sizeof(student_t));
    if(newStudentBytes == -1){
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
    if(newStudentBytes != sizeof(student_t)){
        printf(M_ERR_DB_WRITE);
        return ERR_DB_OP;
    }
...
int del_student(int fd, int id){
    ...
    off_t studentDatabasePosition = lseek(fd, 0, SEEK_SET);
    if(studentDatabasePosition == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    student_t currentStudentEntry;
    ssize_t studentBytesFromDatabaseFile;
    int foundStudent = 0;
    while((studentBytesFromDatabaseFile = read(fd, &currentStudentEntry, sizeof(student_t))) > 0){
        if(currentStudentEntry.id == id){
            if(lseek(fd, studentDatabasePosition, SEEK_SET) == -1){
                printf(M_ERR_DB_READ);
                return ERR_DB_FILE;
            }
            ssize_t emptyStudentByteReplacementToWrite = write(fd, &EMPTY_STUDENT_RECORD, sizeof(student_t));
            if(emptyStudentByteReplacementToWrite == -1){
                printf(M_ERR_DB_WRITE);
                return ERR_DB_FILE;
            }
            foundStudent = 1;
        ...
        studentDatabasePosition =  lseek(fd, 0, SEEK_CUR);
    }
    if(studentBytesFromDatabaseFile == -1){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
...
int count_db_records(int fd){
    ...
    int databaseRecordCounter = 0;
    student_t currentStudentEntry;
    ssize_t currentStudentBytes;
    while((currentStudentBytes = read(fd, &currentStudentEntry, sizeof(student_t))) > 0){
        if(memcmp(&currentStudentEntry, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0){
            databaseRecordCounter++;
        }
    }
    if(currentStudentBytes == -1){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    if(databaseRecordCounter > 0){
        printf(M_DB_RECORD_CNT, databaseRecordCounter);
    }else{
        printf(M_DB_EMPTY);
    }
    return databaseRecordCounter;
}
...
int print_db(int fd){
    ...
    studentDatabasePosition = lseek(fd, 0, SEEK_SET);
    if(studentDatabasePosition == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    ...
    while ((currentStudentBytes = read(fd, &currentStudentEntry, sizeof(student_t))) > 0) {
        if (memcmp(&currentStudentEntry, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            float currentStudentGPA = currentStudentEntry.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, currentStudentEntry.id, currentStudentEntry.fname, currentStudentEntry.lname, currentStudentGPA);
        }
    }
    if (currentStudentBytes == -1) {
        printf(M_ERR_DB_READ);
...
void print_student(student_t *s){
    ...
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
    float currentStudentGPA = s->gpa / 100.0;
    printf(STUDENT_PRINT_FMT_STRING, s->id, s->fname, s->lname, currentStudentGPA);
}
