...
int start_server(char *ifaces, int port, int is_threaded){
    ...
    //
    if(is_threaded){
        printf("Multi-threading not implementing!\n");
    }
...
int send_message_string(int cli_socket, char *buff){
    int currentCommandLength = strlen(buff);
    ssize_t io_size = send(cli_socket, buff, currentCommandLength, 0);
    if(io_size == -1){
        return ERR_RDSH_COMMUNICATION;
    }
    return OK;
